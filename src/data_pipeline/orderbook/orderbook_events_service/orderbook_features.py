...
def weighted_avg_price(df, amount):
    try:
        weighted_average_price = weighted_avg(df[df['cumsum_dollar_dollar_bills'] < amount]['price'], df[df['cumsum_dollar_dollar_bills'] < amount]['volume'])
    except ZeroDivisionError:
        weighted_average_price = df['price'][0]
