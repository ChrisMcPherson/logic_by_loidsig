...
select model_version, trade_datetime, predicted_return, trade_duration
	, ((sell_price - buy_price) / buy_price) * 100 as return_perc
	, ((sell_price - buy_price) / buy_price) * 100 - .15 as return_perc_post_fees
	, sell_price*sell_quantity - buy_price*buy_quantity as return_dollars
	, (sell_price*sell_quantity - buy_price*buy_quantity) - (sell_price*sell_quantity * .00075) - (buy_price*buy_quantity * .00075) as return_dollars_post_fees
	, model_version
	, scoring_close_price
	, buy_price_pre_order
	, buy_price
...
select scoring_results.model_version, scoring_results.target_coin
	...
	, sum((sell_price*sell_quantity - buy_price*buy_quantity) - (sell_price * sell_quantity * .00075) - (buy_price * buy_quantity * .00075)) as return_dollars_post_fees
	, sum(((sell_price - buy_price) / buy_price) * 100) as total_return_perc
	, sum((((sell_price - (sell_price * .00075) - (buy_price * .00075)) - buy_price) / buy_price) * 100) as total_return_perc_after_binance
...
having is_trade = true
and scoring_results.model_version >= 4
;
